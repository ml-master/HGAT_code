...
def evaluate(preds_list, y_list):
    ...
    if multi_label:
        ...
        [precision_ma, recall_ma, F1_ma, support] = \
            precision_recall_fscore_support(y_list[preds.sum(axis=1) != 0], preds[preds.sum(axis=1) != 0],
                                            average='macro')
        Acc = precision_score(y_list,preds,average='micro')*100
        ...
        precision_per_class = precision_score(y_list, preds, average=None)
        fakeAC = precision_per_class[0]  # 负类的精确率
        legAC = precision_per_class[1]  # 正类的精确率
        Acc = precision_score(y_list, preds, average='micro')*100
        print(' Ac: %6.2f' % Acc,
              'fackACC:%5.1f'%(fakeAC*100),
              'legACC:%5.1f'%(legAC*100),
              end="")
        return Acc, legAC*100,fakeAC*100
...
def train(epoch,
    ...
    if 'multi' in dataset:
        acc_val, res_line ,fakeAcc = results
        return float(acc_val), res_line
    else:
        acc_val, trueAcc ,fakeAcc = results
        return float(acc_val), float(trueAcc),fakeAcc
...
def test(epoch, input_adj_test, input_features_test, idx_out_test, idx_test):
    ...
    if 'multi' in dataset:
        ...
        return float(acc_test), res_line
    else:
        acc_test, f1_test ,fakeacc= results
        return float(acc_test), float(f1_test),fakeacc
...
# Model and optimizer
    ...
    for epoch in range(args.epochs):
        vali_acc, vali_ta ,val_fa= train(epoch,
                         input_adj_train, input_features_train, idx_out_train, idx_train,
                         input_adj_val, input_features_val, idx_out_val, idx_val)
        test_acc, test_ta ,test_fa = test(epoch,
                        input_adj_test, input_features_test, idx_out_test, idx_test)
        if vali_acc > vali_max[0]:
            vali_max = [vali_acc, (test_acc, test_ta,test_fa), epoch+1]
    ...
    if 'multi' in dataset:
        print("The best result is ACC: {0:.4f}, {1}where epoch is {2}\n{1}\n".format(
            vali_max[1][0],
            vali_max[1][1],
            vali_max[1][2],
            vali_max[2]))
...
for i in range(len(FINAL_RESULT)):
    if 'multi' in dataset:
        print("{0}:\tvali:  {1:.5f}\ttest:  ACC: {2:.4f},Trueacc: , fakeAcc: epoch={4}.\n{3}".format(
            i,
            FINAL_RESULT[i][0],
            FINAL_RESULT[i][1][0],
            FINAL_RESULT[i][1][1],
        ...
        print("{}:\tvali:  {:.5f}\ttest:  ACC: {:.4f} Trueacc: {:.4f} , fakeAcc:{:.4f}, epoch={}".format(
                        i,
            FINAL_RESULT[i][0],
            FINAL_RESULT[i][1][0],
            FINAL_RESULT[i][1][1],
