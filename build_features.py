...
def build_entity_feature_with_description(datapath, stopwords=list()):
    ...
    vectorizer = CountVectorizer(min_df = 2, stop_words=list(stopwords),max_df=50)
...
def build_text_feature(datapath, DATASETS, rho=0.3, lp=0.5, stopwords=list()):
    ...
    vectorizer = CountVectorizer(min_df=0.001, stop_words=list(stopwords),max_df=0.9)
    X = vectorizer.fit_transform(corpus)
